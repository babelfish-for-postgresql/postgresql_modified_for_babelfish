		 ...
		 lineoff++, lpp++)
	{
		if (ItemIdIsNormal(lpp))
		{
			...
			if (all_visible)
				...
				valid = table_tuple_satisfies_visibility(scan->rs_base.rs_rd, &loctup, snapshot, buffer);
		   ...
		   bool keep_buf)
{
	 ...
	 */
	valid = table_tuple_satisfies_visibility(relation,
											 tuple,
											 snapshot,
											 buffer);
					   ...
					   bool *all_dead, bool first_call)
{
	...
	for (;;)
	{
		...
		if (!skip)
		{
			...
			valid = table_tuple_satisfies_visibility(relation, heapTuple, snapshot, buffer);
					...
					ItemPointer tid)
{
	...
	for (;;)
	{
		 ...
		 * candidate.
		 */
		valid = table_tuple_satisfies_visibility(relation, &tp, snapshot, buffer);
...
l1:
	...
	result = table_tuple_satisfies_update(relation, &tp, cid, buffer);
	...
	if (crosscheck != InvalidSnapshot && result == TM_Ok)
	{
		...
		if (table_tuple_satisfies_visibility(relation, &tp, crosscheck, buffer))
			result = TM_Updated;
	}
...
l2:
	...
	result = table_tuple_satisfies_update(relation, &oldtup, cid, buffer);
	...
	{
		/* Perform additional check for transaction-snapshot mode RI updates */
		if (table_tuple_satisfies_visibility(relation, &oldtup, crosscheck, buffer))
		{
			result = TM_Updated;
...
l3:
	result = table_tuple_satisfies_update(relation, tuple, cid, *buffer);
